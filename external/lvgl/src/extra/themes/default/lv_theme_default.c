...
typedef struct {
    ...
    lv_style_t bg_color_orange;
    // stonewang
    lv_style_t bg_color_dark_kbd_btn;
    lv_style_t bg_color_dark_kbd_pressed_btn;
    lv_style_t pressed;
    lv_style_t disabled;
    lv_style_t pad_zero;
    lv_style_t pad_tiny;
...
static void style_init(void)
{
    ...
    style_init_reset(&styles->bg_color_orange);
    lv_style_set_bg_color(&styles->bg_color_orange, lv_color_hex(0xF5870A));
    lv_style_set_bg_opa(&styles->bg_color_orange, LV_OPA_COVER);
    lv_style_set_text_color(&styles->bg_color_orange, lv_color_hex(0xFFFFFF));
    ...
    // stonewang
    lv_color_t color = lv_color_make(0x66, 0x66, 0x66);
    style_init_reset(&styles->bg_color_dark_kbd_btn);
    lv_style_set_bg_color(&styles->bg_color_dark_kbd_btn, color);
    lv_style_set_bg_opa(&styles->bg_color_dark_kbd_btn, LV_OPA_COVER);
    color = lv_color_make(0xFF, 0xFF, 0xFF);
    lv_style_set_text_color(&styles->bg_color_dark_kbd_btn, color);
#if 0

    style_init_reset(&styles->bg_color_dark_kbd_pressed_btn);
    color = lv_color_make(0xCC, 0xCC, 0xCC);
    lv_style_set_bg_color(&styles->bg_color_dark_kbd_pressed_btn, color);
    lv_style_set_bg_opa(&styles->bg_color_dark_kbd_pressed_btn, LV_OPA_COVER);
    color = lv_color_make(0xFF, 0xFF, 0xFF);
    lv_style_set_text_color(&styles->bg_color_dark_kbd_pressed_btn, color);
#endif
    ...
    // stonewang darkmode
    if (1) {
        lv_style_set_bg_color(&styles->keyboard_btn_bg, lv_color_hex(0x727272));
        lv_style_set_text_color(&styles->keyboard_btn_bg, lv_color_hex(0xffffff));
    } else {

    }
    lv_style_set_radius(&styles->keyboard_btn_bg, disp_size == DISP_SMALL ? RADIUS_DEFAULT / 2 : RADIUS_DEFAULT);

    style_init_reset(&styles->keyboard_disable);
    lv_style_set_color_filter_dsc(&styles->keyboard_disable, &dark_filter);
    lv_style_set_color_filter_opa(&styles->keyboard_disable, LV_OPA_40);
#endif

#if LV_USE_TABVIEW
    style_init_reset(&styles->tab_btn);
    // stone wang
    lv_style_set_bg_opa(&styles->tab_btn, LV_OPA_0);
    lv_style_set_border_width(&styles->tab_btn, BORDER_WIDTH);
    lv_style_set_border_color(&styles->tab_btn, lv_color_hex(0xF5870A));
...
static void theme_apply(lv_theme_t * th, lv_obj_t * obj)
{
    ...
    // stonewang
        ...
        lv_obj_add_style(obj, &styles->outline_secondary, LV_STATE_EDITED);
        lv_obj_add_style(obj, &styles->btn, LV_PART_ITEMS);
        lv_obj_add_style(obj, &styles->keyboard_disable, LV_PART_ITEMS | LV_STATE_DISABLED);
        lv_obj_add_style(obj, &styles->bg_color_white, LV_PART_ITEMS);
        lv_obj_add_style(obj, &styles->keyboard_btn_bg, LV_PART_ITEMS);
        lv_obj_add_style(obj, &styles->pressed, LV_PART_ITEMS | LV_STATE_PRESSED);
        lv_obj_add_style(obj, &styles->bg_color_orange, LV_PART_ITEMS | LV_STATE_CHECKED);
        lv_obj_add_style(obj, &styles->bg_color_dark_kbd_btn, LV_PART_ITEMS | LV_STATE_USER_1);
        lv_obj_add_style(obj, &styles->bg_color_secondary_muted, LV_PART_ITEMS | LV_STATE_EDITED);
//        lv_obj_add_style(obj, &styles->bg_color_primary_muted, LV_PART_ITEMS | LV_STATE_FOCUS_KEY);
