...
impl NamedChain {
    fn str_to_chain_id(s: &str) -> crate::errors::Result<ChainId> {
    ...
    pub fn from_chain_id(chain_id: &ChainId) -> crate::errors::Result<NamedChain> {
        match chain_id.id() {
            ...
            _ => Err(crate::errors::AptosError::ParseTxError(
                "Not a named chain".to_string(),
            )),
        }
    }
}
    ...
    fn from_str(s: &str) -> crate::errors::Result<Self> {
        if s.is_empty() {
            return Err(crate::errors::AptosError::ParseTxError(
                "Cannot create chain ID from empty string".to_string(),
            ));
        }
