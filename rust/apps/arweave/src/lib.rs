...
use third_party::rsa::{RsaPrivateKey, BigUint};
...
pub fn generate_secret(seed: &[u8]) -> Result<RsaPrivateKey> {
    match get_rsa_secret_from_seed(seed) {
        Ok(secret) => Ok(secret),
        Err(e) => Err(ArweaveError::KeystoreError(format!("generate secret failed {:?}", e.to_string()))),
    }
}
...
mod tests {
    ...
    fn test_generate_address() {
        ...
        let address = generate_address(result.n().to_owned().to_bytes_be()).unwrap();
