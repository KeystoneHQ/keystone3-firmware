...
pub fn aes128_encrypt(key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    ...
    for _ in 0..padding {
        encrypted_data.push(padding as u8);
    }
...
pub fn aes128_decrypt(key: &[u8], data: &[u8]) -> Result<Vec<u8>> {
    ...
    let padding = decrypted_data[decrypted_data.len() - 1] as usize;
    decrypted_data.truncate(decrypted_data.len() - padding);
    Ok(decrypted_data)
}
...
mod tests {
    ...
    #[test]
    fn test_aes128() {
        let key = hex::decode("5eb00bbddcf069084889a8ab91555681").unwrap();
        ...
        let encrypted_data = aes128_encrypt(key.as_slice(), data.as_slice()).unwrap();
        let decrypted_data = aes128_decrypt(key.as_slice(), encrypted_data.as_slice()).unwrap();
