...
macro_rules! negative_check {
    ($t: expr, $v: expr) => {{
        ...
        if $v < 0 {
            result = Err(AvaxError::InvalidRawTxCryptoBytes(format!(
                "negative value {:?} for {:?}",
                $v, $t
            )));
        };
...
impl TxIn {
    ...
    pub fn script_sig(
        ...
        let pubkey = PublicKey::from_str(self.pubkey.as_str())
            .map_err(|_| AvaxError::SignLegacyTxError("invalid public key".to_string()))?;
        match script_type {
            ScriptType::P2PKH => {
                let sig_bytes = sig
                    .serialize_der()
            ...
            _ => Err(AvaxError::SignLegacyTxError(
...
impl TryFrom<Input> for TxIn {
    ...
    fn try_from(value: protoc::Input) -> Result<Self> {
        let utxo = value
            .utxo
            .ok_or(AvaxError::InvalidRawTxCryptoBytes(format!("empty utxo")))?;
...
impl InputConverter for protoc::sign_transaction::Transaction {
    fn to_tx_in(&self) -> Result<Vec<TxIn>> {
        let inputs = match self {
            ...
            _ => Err(AvaxError::InvalidRawTxCryptoBytes(
