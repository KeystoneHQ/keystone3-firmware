...
use cardano_serialization_lib::{Ed25519Signature, FixedTransaction, PublicKey, Vkey, Vkeywitness};
...
pub const MAX_CONTEXT_UTXOS: usize = 100;

fn validate_transaction_limits(tx: &[u8], context: &ParseContext) -> Result<()> {
    if tx.len() > MAX_TRANSACTION_SIZE {
        return Err(CardanoError::InvalidTransaction(
            "transaction size exceeds 16 KiB limit".to_string(),
        ));
    }
    if context.utxos_len() > MAX_CONTEXT_UTXOS {
        return Err(CardanoError::InvalidTransaction(
            "UTXO count exceeds 100 limit".to_string(),
        ));
    }
    Ok(())
}
...
pub fn parse_tx(tx: Vec<u8>, context: ParseContext) -> Result<ParsedCardanoTx> {
    validate_transaction_limits(&tx, &context)?;
    let cardano_tx = FixedTransaction::from_bytes(tx)?;
    ParsedCardanoTx::from_cardano_tx(cardano_tx, context)
}

pub fn parse_sign_data(
    sign_data: Vec<u8>,
...
pub fn check_tx(tx: Vec<u8>, context: ParseContext) -> Result<()> {
    validate_transaction_limits(&tx, &context)?;
    let cardano_tx = FixedTransaction::from_bytes(tx)?;
    ParsedCardanoTx::validate_native_asset_limits(&cardano_tx)?;
...
pub fn sign_tx(tx: Vec<u8>, context: ParseContext, icarus_master_key: XPrv) -> Result<Vec<u8>> {
    validate_transaction_limits(&tx, &context)?;
    let cardano_tx = FixedTransaction::from_bytes(tx)?;
    let hash = blake2b_256(cardano_tx.raw_body_ref().as_ref());
    let mut witness_set = cardano_serialization_lib::TransactionWitnessSet::new();
    let mut vkeys = cardano_serialization_lib::Vkeywitnesses::new();
...
mod test {
    ...
    fn spike_fixed_transaction() {
        ...
        let tx = FixedTransaction::from_bytes(tx_bytes).unwrap();
        let body = tx.raw_body_ref();
