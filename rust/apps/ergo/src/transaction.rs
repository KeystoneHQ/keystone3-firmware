...
pub fn sign_tx(tx: Vec<u8>, seed: &[u8], entropy: &[u8], derivation_paths: &Vec<String>) -> Result<Vec<u8>> {
    let reduced_ergo_tx = ReducedTransaction::sigma_parse_bytes(tx.clone().as_mut_slice())
        .map_err(|e| ErgoError::TransactionParseError(e.to_string()))?;
   ...
   let wallet = Wallet::from_secrets(secrets);
    ...
    let signed_tx: Transaction = wallet.sign_reduced_transaction_deterministic(reduced_ergo_tx, entropy)
        .map_err(|e| ErgoError::SigningFailed(e.to_string()))?;

    signed_tx.sigma_serialize_bytes().map_err(|e| ErgoError::SigningFailed(e.to_string()))
}
