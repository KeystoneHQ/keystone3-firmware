...
use crate::utils::{
    decrypt_data_with_pvk, encrypt_data_with_pvk, hash_to_scalar, DecryptUrData,
    KEY_IMAGE_EXPORT_MAGIC, OUTPUT_EXPORT_MAGIC, PUBKEY_LEH, SIGNED_TX_PREFIX,
};
...
impl KeyImage {
    pub fn new(image: [u8; PUBKEY_LEH], signature: [u8; 64]) -> KeyImage {
        KeyImage { image, signature }
    }
}
...
impl KeyImages {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        for key_image in self.0.iter() {
            data.extend_from_slice(&key_image.image);
...
pub fn generate_export_ur_data(keypair: KeyPair, request_data: Vec<u8>) -> Vec<u8> {
    let decrypted_data: DecryptUrData = decrypt_data_with_pvk(
    ...
    let outputs = crate::outputs::ExportedTransferDetails::from_bytes(&decrypted_data.data);

    let mut key_images: KeyImages = KeyImages(vec![]);
    let rng_seed = crate::utils::keccak256(request_data.as_slice());
    let mut rng = rand_chacha::ChaCha20Rng::from_seed(rng_seed.try_into().unwrap());
    for output in outputs.details.iter() {
        key_images
            .0
            .push(output.generate_key_image(&keypair.clone(), &mut rng));
    }

    encrypt_data_with_pvk(keypair, key_images.to_bytes(), KEY_IMAGE_EXPORT_MAGIC)
}
...
mod tests {
    ...
    fn test_parse_keyimages() {
        ...
        let res: DecryptUrData =
            decrypt_data_with_pvk(pvk.try_into().unwrap(), data, KEY_IMAGE_EXPORT_MAGIC);
        ...
        assert_eq!(
            hex::encode(key_images.get(7).image),
