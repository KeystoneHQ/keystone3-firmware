...
impl TxConstructionData {
    fn should_use_additional_keys(&self) -> bool {
        self.sources.iter().any(|source| source.real_out_additional_tx_keys.len() > 0)
    }
    ...
    pub fn transaction_keys(&self) -> (PrivateKey, Vec<PrivateKey>, EdwardsPoint, Vec<EdwardsPoint>) {
        ...
        if self.should_use_additional_keys() {
            for _ in 0 .. self.splitted_dsts.len() {
              additional_keys.push(
                  PrivateKey::from_bytes(generate_random_scalar(&mut rng).as_bytes())
              );
            }
        }
        ...
        if has_payments_to_subaddresses && !should_use_additional_keys {
            let spend = self.splitted_dsts.iter().find(|dest| dest.is_subaddress).unwrap().addr.spend_public_key;

            tx_key_pub = tx_key.scalar * PublicKey::from_bytes(&spend).unwrap().point.decompress().unwrap();
        } else if should_use_additional_keys {
            for (additional_key, dest) in additional_keys.clone().into_iter().zip(&self.splitted_dsts) {
                let spend = PublicKey::from_bytes(&dest.addr.spend_public_key).unwrap();
                if dest.is_subaddress {
                    additional_keys_pub.push(additional_key.scalar * spend.point.decompress().unwrap());
                } else {
    ...
    fn ecdhs(&self, keypair: &KeyPair) -> Vec<EdwardsPoint> {
        ...
        for (i, dest) in self.splitted_dsts.iter().enumerate() {
            let key_to_use =
                if dest.is_subaddress { additional_keys.get(i).unwrap_or(&tx_key) } else { &tx_key };
            res.push(if !self.is_change_dest(dest) {
              key_to_use.scalar * PublicKey::from_bytes(&dest.addr.view_public_key).unwrap().point.decompress().unwrap()
            } else {
    ...
    pub fn sum_output_masks(&self, keypair: &KeyPair) -> Scalar {
      self
        .commitments_and_encrypted_amounts(keypair)
        .into_iter()
