...
pub const GAS_DIVIDER: f64 = 1_000_000_000_000_f64;
...
where
    S: Serializer,
{
    serializer.serialize_str(&format_amount(v.unwrap_or_default()))
}
...
where
    S: Serializer,
{
    serializer.serialize_str(&format_amount(*v))
}
...
where
    S: Serializer,
{
    serializer.serialize_str(&format_gas(*v))
}
