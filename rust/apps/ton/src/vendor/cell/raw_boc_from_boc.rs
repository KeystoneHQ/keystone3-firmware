...
fn build_and_verify_index(roots: &[ArcCell]) -> BTreeMap<CellHash, RefCell<IndexedCell>> {
    let mut current_cells: Vec<_> = roots.iter().map(Arc::clone).collect();
    let mut new_hash_index = 0;
    let mut cells_by_hash: BTreeMap<CellHash, RefCell<IndexedCell>> = BTreeMap::new();
...
fn root_indices(
    roots: &[ArcCell],
    cells_dict: &BTreeMap<CellHash, RefCell<IndexedCell>>,
...
fn raw_cells_from_cells(
    ...
    cells_by_hash: &BTreeMap<CellHash, RefCell<IndexedCell>>,
