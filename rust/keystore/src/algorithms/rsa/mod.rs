...
pub fn sign_message(data: &[u8], p: &[u8], q: &[u8], signing_option: &SigningOption) -> Result<Vec<u8>> {
...
pub fn build_rsa_private_key_from_primes(p: &[u8], q: &[u8]) -> Result<RsaPrivateKey> {
    ...
    let d = e.clone().mod_inverse((p.clone() - 1u8) * (q.clone() - 1u8)).ok_or_else(|| {
        KeystoreError::GenerateSigningKeyError("failed to calculate rsa private key".to_string())
    })?;
    let private_key = RsaPrivateKey::from_components(n, e, d.to_biguint().unwrap(), vec![p, q]).map_err(|_| {
...
mod tests {
    ...
    fn test_sign_message() {
        ...
        let result = sign_message(data, p.as_slice(), q.as_slice(), &SigningOption::PSS { salt_len: 32 }).unwrap();
    ...
    fn test_sign_verify_salt_zero() {
        ...
        let seed = hex::decode("5eb00bbddcf069084889a8ab9155568165f5c453ccb85e70811aaed6f6da5fc19a5ac40b389cd370d086206dec8aa6c43daea6690f20ad3d8d48b2d2ce9e38e4").unwrap();
        let signature = sign_message(message.as_slice(), p.as_slice(), q.as_slice(), &signing_option).unwrap();
