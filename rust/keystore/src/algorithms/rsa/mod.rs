...
pub fn get_rsa_secret_from_seed(seed: &[u8]) -> Result<RsaPrivateKeyParts> {
    ...
    let p = hex::encode(private_key.primes()[0].to_bytes_be());
    let q = hex::encode(private_key.primes()[1].to_bytes_be());
    let d = hex::encode(private_key.d().to_bytes_be());
    let n = hex::encode(private_key.n().to_bytes_be());
...
mod tests {
    ...
    fn test_private_key_recover() {
        ...
        assert_eq!(result.p, hex::encode(p));
        assert_eq!(result.q, hex::encode(q));
        assert_eq!(result.d, hex::encode(d));
        assert_eq!(result.n, hex::encode(n));
    }
}
