...
fn prepare_parse_context(
    ergo_sign_request: &ErgoSignRequest,
    x_pub: String,
) -> R<ParseContext> {
    let utxo: Result<Vec<ErgoUnspentBox>, ErgoError> = ergo_sign_request.get_boxes()
        .iter()
        .map(|ergo_box| {
            match app_ergo::address::ergo_tree_to_address(ergo_box.get_ergo_tree()) {
                Ok(address) => Ok(ErgoUnspentBox::new(
                    ...
                    match ergo_box.get_assets() {
                        None => None,
                        Some(v) => Some(
                            v.iter().map(|a| UnparsedErgoAsset::new(a.get_token_id(), a.get_amount())).collect_vec()
                        )
                    }
                )),
                Err(e) => Err(e)
    ...
    let addresses = ergo_sign_request.get_derivation_paths()
        .iter()
        .map(|derivation_path| derivation_path.get_path())
        .flatten()
        .map(|derivation_path| app_ergo::address::get_address(format!("m/{}",derivation_path), &x_pub))
