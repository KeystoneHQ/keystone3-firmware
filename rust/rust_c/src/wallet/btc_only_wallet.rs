...
#[no_mangle]
pub unsafe extern "C" fn get_connect_blue_wallet_ur(
    ...
    let key2 = keys.get(1);
    let key3 = keys.get(2);
    if let (Some(k1), Some(k2), Some(k3)) = (key1, key2, key3) {
...
#[no_mangle]
pub unsafe extern "C" fn get_connect_sparrow_wallet_ur(
    master_fingerprint: *mut u8,
    length: u32,
    public_keys: PtrT<CSliceFFI<ExtendedPublicKey>>,
) -> *mut UREncodeResult {
    ...
    let key3 = keys.get(2);
    let key4 = keys.get(3);
    if let (Some(k1), Some(k2), Some(k3), Some(k4)) = (key1, key2, key3, key4) {
        let k1_x_pub = recover_c_char(k1.xpub);
        let k1_x_pub_path = recover_c_char(k1.path);
        let k2_x_pub = recover_c_char(k2.xpub);
        let k2_x_pub_path = recover_c_char(k2.path);
...
impl AccountConfig {
    pub unsafe fn into(&self) -> app_wallets::companion_app::AccountConfig {
...
impl CoinConfig {
    pub unsafe fn into(&self) -> app_wallets::companion_app::CoinConfig {
...
#[no_mangle]
pub unsafe extern "C" fn get_connect_companion_app_ur(
...
#[no_mangle]
pub unsafe extern "C" fn get_okx_wallet_ur_btc_only(
    master_fingerprint: PtrBytes,
