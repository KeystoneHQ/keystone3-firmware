...
fn encrypt_ufvk_blob_with_magic(data: &[u8], key: &[u8], iv: &[u8], magic: &str) -> String {
    let iv_generic = GenericArray::from_slice(iv);
    let key = GenericArray::from_slice(key);
    let ct = Aes256CbcEnc::new(key, iv_generic).encrypt_padded_vec_mut::<Pkcs7>(data);
    format!("{}{}{}", magic, hex::encode(iv), hex::encode(ct))
}

fn decrypt_ufvk_blob_with_magic(
    blob: &[u8],
    key: &[u8],
    magic: &[u8],
) -> Result<String, RustCError> {
    if !blob.starts_with(magic) {
        return Err(RustCError::InvalidHex("invalid blob magic".to_string()));
    }
    let payload = &blob[magic.len()..];
    if payload.len() < 2 * 16 {
        return Err(RustCError::InvalidHex("invalid blob".to_string()));
    }
    let (iv_hex, ct_hex) = payload.split_at(2 * 16);
    let iv = hex::decode(iv_hex).map_err(|_| RustCError::InvalidHex("invalid iv".to_string()))?;
    let mut data = hex::decode(ct_hex)
        .map_err(|_| RustCError::InvalidHex("invalid ciphertext".to_string()))?;
    let iv = GenericArray::from_slice(&iv);
    let key = GenericArray::from_slice(key);
    let result = Aes256CbcDec::new(key, iv)
        .decrypt_padded_vec_mut::<Pkcs7>(&data)
        .map_err(|_| RustCError::InvalidHex("decrypt failed".to_string()))
        .and_then(|mut plaintext| {
            let result = core::str::from_utf8(&plaintext)
                .map(|value| value.to_string())
                .map_err(|_| RustCError::InvalidHex("invalid plaintext".to_string()));
            plaintext.zeroize();
            result
        });
    data.zeroize();
...
pub unsafe extern "C" fn rust_encrypt_ufvk_blob(
    ...
    let mut data = unsafe { recover_c_char(data) };
    let key = extract_array!(key, u8, key_len as usize);
    let iv = extract_array!(iv, u8, iv_len as usize);
    ...
    let value = encrypt_ufvk_blob_with_magic(data.as_bytes(), key, iv, "z2");
    data.zeroize();
    SimpleResponse::success(convert_c_char(value)).simple_c_ptr()
}

#[no_mangle]
pub unsafe extern "C" fn rust_encrypt_ufvk_blob_v3(
    data: PtrString,
    key: PtrBytes,
    key_len: u32,
    iv: PtrBytes,
    ...
    let key = extract_array!(key, u8, key_len as usize);
    let iv = extract_array!(iv, u8, iv_len as usize);
    let value = encrypt_ufvk_blob_with_magic(data.as_bytes(), key, iv, "z3");
    data.zeroize();
    SimpleResponse::success(convert_c_char(value)).simple_c_ptr()
}
...
pub unsafe extern "C" fn rust_decrypt_ufvk_blob(
    ...
    let key = extract_array!(key, u8, key_len as usize);
    match decrypt_ufvk_blob_with_magic(blob.as_bytes(), key, b"z2") {
        Ok(mut plaintext) => {
            let result = convert_c_char(plaintext.clone());
            plaintext.zeroize();
            SimpleResponse::success(result).simple_c_ptr()
        }
        Err(error) => SimpleResponse::from(error).simple_c_ptr(),
    }
}

#[no_mangle]
pub unsafe extern "C" fn rust_decrypt_ufvk_blob_v3(
    blob: PtrString,
    key: PtrBytes,
    key_len: u32,
) -> *mut SimpleResponse<c_char> {
    let blob = unsafe { recover_c_char(blob) };
    let key = extract_array!(key, u8, key_len as usize);
    match decrypt_ufvk_blob_with_magic(blob.as_bytes(), key, b"z3") {
        Ok(mut plaintext) => {
            let result = convert_c_char(plaintext.clone());
            plaintext.zeroize();
            SimpleResponse::success(result).simple_c_ptr()
...
mod tests {
    ...
    #[test]
    fn test_aes256_ufvk_blob_v3_round_trip() {
        let data = convert_c_char("hello world".to_string());
        let mut key = [0x42u8; 32];
        let mut iv = [0x24u8; 16];

        let encrypted = unsafe {
            rust_encrypt_ufvk_blob_v3(
                data,
                key.as_mut_ptr(),
                key.len() as u32,
                iv.as_mut_ptr(),
        ...
        assert!(!unsafe { (*encrypted).data.is_null() });
        let blob = unsafe { recover_c_char((*encrypted).data) };
        assert!(blob.starts_with("z3"));

        let blob_input = convert_c_char(blob);
        let decrypted =
            unsafe { rust_decrypt_ufvk_blob_v3(blob_input, key.as_mut_ptr(), key.len() as u32) };
        assert!(!decrypted.is_null());
        assert!(!unsafe { (*decrypted).data.is_null() });
        assert_eq!(unsafe { recover_c_char((*decrypted).data) }, "hello world");
    }

    #[test]
    fn test_aes256_ufvk_blob_v3_rejects_wrong_key_and_z2_blob() {
        let data = convert_c_char("hello world".to_string());
        let mut key = [0x42u8; 32];
        let mut wrong_key = [0x43u8; 32];
        let mut iv = [0x24u8; 16];
        ...
        let blob = unsafe { recover_c_char((*encrypted).data) };
        let wrong_key_input = convert_c_char(blob);
        let wrong_key_result = unsafe {
            rust_decrypt_ufvk_blob_v3(
                wrong_key_input,
                wrong_key.as_mut_ptr(),
                wrong_key.len() as u32,
            )
        };
        assert!(!wrong_key_result.is_null());
        assert!(unsafe { (*wrong_key_result).data.is_null() });

        let z2_input = convert_c_char(
            "z22424242424242424242424242424242400112233445566778899aabbccddeeff".to_string(),
        );
        let z2_result =
            unsafe { rust_decrypt_ufvk_blob_v3(z2_input, key.as_mut_ptr(), key.len() as u32) };
        assert!(!z2_result.is_null());
        assert!(unsafe { (*z2_result).data.is_null() });
    }
