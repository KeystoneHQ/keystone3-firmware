...
static lv_obj_t *g_tooltipHintBox = NULL;
...
{
    if (g_tooltipHintBox != NULL) {
        lv_obj_del(g_tooltipHintBox);
        g_tooltipHintBox = NULL;
    }
...
void GuiCreateTooltipHintBox(char *titleText, char *descText, char *link)
{
    ...
    if (link != NULL) {
        height += 12;
        lv_obj_t *learnMoreCont = GuiCreateContainerWithParent(g_tooltipHintBox, 144, 30);
    ...
    if (descText != NULL) {
        height += 12;
        desc = GuiCreateNoticeLabel(g_tooltipHintBox, descText);
        if (linkLabel != NULL) {
    ...
    if (titleText != NULL) {
        title = GuiCreateTextLabel(g_tooltipHintBox, titleText);
        if (desc != NULL) {
            lv_obj_align_to(title, desc, LV_ALIGN_OUT_TOP_LEFT, 0, -12);
        } else {
    ...
    height += 114;
    lv_obj_t *rightBtn = GuiCreateTextBtn(g_tooltipHintBox, _("got_it"));
    lv_obj_align(rightBtn, LV_ALIGN_BOTTOM_RIGHT, -36, -24);
    ...
    lv_obj_add_event_cb(rightBtn, CloseTooltipHintBoxHandler, LV_EVENT_CLICKED, g_tooltipHintBox);

    GuiHintBoxResize(g_tooltipHintBox, height);
}
