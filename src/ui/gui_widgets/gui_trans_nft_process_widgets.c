...
const GuiMsgBox_t g_guiMsgBoxTransNftProcess = {
...
static void GuiFirmwareProcessInit(void)
{
    ...
    label = GuiCreateTextLabel(container, _("nft_trans_title"));
    lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 403);
    label = GuiCreateNoticeLabel(container, _("nft_trans_updating_desc"));
    lv_obj_align(label, LV_ALIGN_TOP_MID, 0, 457);
    label = GuiCreateNoticeLabel(container, _("nft_trans_updating_hint"));
