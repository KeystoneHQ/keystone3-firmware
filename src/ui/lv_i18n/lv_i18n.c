...
const static lv_i18n_phrase_t de_singulars[] = {
    ...
    {"scan_qr_code_signing_hash_signing_desc", " um den Hash einer Transaktion für Integrität und Authentizität zu signieren. Dies können Sie mit der Software-Wallet vergleichen."},
...
const static lv_i18n_phrase_t es_singulars[] = {
    ...
    {"scan_qr_code_signing_hash_signing_desc", "La firma de hash utiliza una clave privada para firmar el hash de una transacción para asegurar su integridad y autenticidad. Puedes comparar esto con la cartera de software. Das Hash-Signieren verwendet einen privaten Schlüssel"},
...
const static lv_i18n_phrase_t ja_singulars[] = {
    ...
    {"scan_qr_code_signing_hash_signing_desc", "ハッシュ署名は、トランザクションのハッシュを署名して整合性と真正性を保証するために秘密鍵を使用します。これをソフトウェアウォレットと比較することができます。"},
...
const static lv_i18n_phrase_t ko_singulars[] = {
    ...
    {"scan_qr_code_signing_hash_signing_desc", "해시 서명은 거래의 해시에 대한 무결성과 진위를 인증하기 위해 개인 키를 사용합니다. 이를 소프트웨어 지갑과 비교할 수 있습니다."},
...
const static lv_i18n_phrase_t ru_singulars[] = {
    ...
    {"scan_qr_code_signing_hash_signing_desc", "Хешированная подпись использует закрытый ключ для подписания хеша транзакции для обеспечения целостности и подлинности. Вы можете сравнить это с программным кошельком."},
...
const static lv_i18n_phrase_t zh_cn_singulars[] = {
    ...
    {"scan_qr_code_signing_hash_signing_desc", "哈希签名使用私钥对交易的哈希值进行签名，以确保数据的完整性和真实性。您可以将此与软件钱包进行比较。"},
