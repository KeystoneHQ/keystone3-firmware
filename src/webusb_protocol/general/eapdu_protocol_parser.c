...
typedef struct {
    uint32_t len;
    uint8_t data[MAX_PACKETS_LENGTH];
} EapduSendJob_t;

static int32_t SendPacketAsync(const void *inData, uint32_t inDataLen)
{
    if (inData == NULL || inDataLen != sizeof(EapduSendJob_t)) {
        return -1;
    }

    const EapduSendJob_t *job = (const EapduSendJob_t *)inData;
    if (g_sendFunc == NULL) {
        return -1;
    }

    g_sendFunc(job->data, job->len);
    return SUCCESS_CODE;
}
...
void SendEApduResponse(EAPDUResponsePayload_t *payload)
{
    ...
    while (payload->dataLen > 0) {
        ...
        if (g_sendFunc) {
            EapduSendJob_t sendJob = {0};
            sendJob.len = OFFSET_CDATA + packetDataSize + EAPDU_RESPONSE_STATUS_LENGTH;
            memcpy_s(sendJob.data, sizeof(sendJob.data), packet, sendJob.len);
            AsyncExecute(SendPacketAsync, &sendJob, sizeof(sendJob));
        } else {
        }
